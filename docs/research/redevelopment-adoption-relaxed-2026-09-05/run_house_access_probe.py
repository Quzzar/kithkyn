...
run.mkdir();shutil.copytree(ROOT/'seed-housing/world',run/'world')
...
cmd=ns['command']('',True);cmd=[c.replace(str(ROOT/'runtime/build/classes/java/main'),str(DIAG/'classes')) for c in cmd];cmd.insert(1,'-Dkithkyn.accessProbe='+MODE)
...
for l in txt.splitlines():
 if '[house-access]' in l:print(l)
raise SystemExit(0 if '[house-access] RESULT PASS' in txt else 1)
