...
public final class HouseAccessProbe {
 static int ticks;static RealPerson builder;static Village village;static StructureInProgress project;
 ...
 @SubscribeEvent public static void tick(ServerTickEvent.Post event) {
  ...
  try {
   ...
   if(ticks==80){
    ...
    goals.removeAllGoals(goal->{try{return !(goal instanceof OpenDoorGoal) && !(goal instanceof WorkLoopGoal<?> && stepField.get(goal) instanceof GatherStep);}catch(Exception e){throw new RuntimeException(e);}});
    ...
    server.tickRateManager().requestGameToSprint(1200);
   }
   if(builder!=null && ticks%40==0){var path=builder.getNavigation().getPath();Kithkyn.LOGGER.info("[house-access] tick={} pos={} velocity={} horizontalCollision={} verticalCollision={} onGround={} door={} progress={} pathIndex={} pathCount={}",ticks-80,builder.position(),builder.getDeltaMovement(),builder.horizontalCollision,builder.verticalCollision,builder.onGround(),level.getBlockState(new BlockPos(129,6,131)),project.getProgress(),path==null?-1:path.getNextNodeIndex(),path==null?-1:path.getNodeCount());}
   if(builder!=null && (!project.isGathering() || ticks>=1280)){
    Kithkyn.LOGGER.info("[house-access] RESULT {} mode={} ticks={} progress={} position={}",project.isGathering()?"FAIL":"PASS",MODE,ticks-80,project.getProgress(),builder.position());server.tickRateManager().stopSprinting();server.halt(false);builder=null;
   }
