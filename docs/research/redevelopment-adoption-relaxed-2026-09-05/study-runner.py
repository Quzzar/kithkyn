...
ORDER=['housing-on','housing-off','opportunity-on','opportunity-off','growth-on','growth-off']
...
try:
 for scenario in ['housing','opportunity','growth']:
  name='seed-'+scenario;run=ROOT/name
  if state['jobs'].get(name,{}).get('status')=='complete':continue
  run.mkdir(exist_ok=True);config(run,False,scenario!='growth');execute(state,name,run,name,False)
 manifest=json.loads((ROOT/'manifest.json').read_text())
 manifest['seedLevelDataSha256']={scenario:digest(ROOT/('seed-'+scenario)/'world/level.dat') for scenario in ['housing','opportunity','growth']}
 (ROOT/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
 state['status']='running';save(state)
 for name in ORDER:
  if state['jobs'].get(name,{}).get('status')=='complete':continue
  scenario,flag=name.split('-');run=ROOT/name
  if not run.exists():
   run.mkdir();shutil.copytree(ROOT/('seed-'+scenario)/'world',run/'world')
  config(run,True,scenario!='growth');execute(state,name,run,'timelapse',flag=='on')
