...
| Children | Working. Toddler and Kid use distinct multipliers over Minecraft's naturally smaller young-player model; Kid is 5% smaller than its former size. Teenager uses adult-player geometry scaled to about 93%, avoiding young-model proportions. Nameplates follow display height, while collision height has a separate age-based cap. Idle pre-adults use commonwear, while an employed Teenager uses occupation clothing. See [families.md](families.md). |
