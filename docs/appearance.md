...
| Children | Working. Toddler and Kid use distinct multipliers over Minecraft's naturally smaller young-player model; both are 5% smaller than their former sizes. Teenager uses adult-player geometry scaled to about 93%, avoiding young-model proportions. Nameplates follow display height, while collision height has a separate age-based cap. Idle pre-adults use commonwear, while an employed Teenager uses occupation clothing. See [families.md](families.md). |
