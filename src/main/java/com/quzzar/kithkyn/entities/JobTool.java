...
public enum JobTool {
  ...
  SWORD(SwordItem.class, Items.STONE_SWORD, 2, 0),
  BOW(BowItem.class, Items.BOW, 0, 2),
  CROSSBOW(CrossbowItem.class, Items.CROSSBOW, 0, 3),
  ...
  /** A fresh copy of the one-time basic starting tool for this post. */
  public ItemStack basicStack() {
    return new ItemStack(this.basic);
  }
  ...
  public static void replace(RealPerson person, JobTool tool, @Nullable BlockPos depositToLoc) {
    ...
    if (made.isEmpty()) {
      ...
      Kithkyn.LOGGER.info("'{}' has no {} and the stores hold nothing to make one from",
  ...
  public static ItemStack makeFromPack(RealPerson person, JobTool tool) {
    for (Item candidate : tool.candidatesBestFirst()) {
      ...
      Kithkyn.LOGGER.info("'{}' made a {} from {} on the road", person.getFullName(), plain(candidate),
          describe(spent));
  ...
  private ItemStack make(RealPerson person, @Nullable BlockPos near, Item product, List<Item> headMaterials) {
    ...
    Kithkyn.LOGGER.info("'{}' made a {} from {} for tomorrow's work", person.getFullName(),
