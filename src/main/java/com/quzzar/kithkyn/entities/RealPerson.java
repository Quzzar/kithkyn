...
import com.quzzar.kithkyn.networking.OpenPersonChatPacket;
...
import com.quzzar.kithkyn.entities.ai.goals.work.BonemealStep;
import com.quzzar.kithkyn.entities.ai.goals.work.ChopStep;
import com.quzzar.kithkyn.entities.ai.goals.work.CookStep;
import com.quzzar.kithkyn.entities.ai.goals.work.HarvestStep;
import com.quzzar.kithkyn.entities.ai.goals.work.TillStep;
import com.quzzar.kithkyn.entities.ai.goals.work.HaulStep;
import com.quzzar.kithkyn.entities.ai.goals.work.CraftStep;
import com.quzzar.kithkyn.entities.ai.goals.work.CullStep;
...
import com.quzzar.kithkyn.entities.ai.goals.work.HerdStep;
import com.quzzar.kithkyn.entities.ai.goals.work.PathStep;
import com.quzzar.kithkyn.entities.ai.goals.work.GradeStep;
...
import com.quzzar.kithkyn.entities.ai.goals.PersonMeleeGoal;
import com.quzzar.kithkyn.entities.ai.goals.RunToEatGoal;
import com.quzzar.kithkyn.entities.ai.goals.SearchForItemsGoal;
import com.quzzar.kithkyn.entities.ai.goals.SetRunningToEatGoal;
import com.quzzar.kithkyn.entities.ai.goals.PanicToBedGoal;
...
public class RealPerson extends Person {
   ...
   * the day's supports to the stores, and the restock carries a cave's worth back
   * out from bed rather than risking a fetch trip from deep in the shaft.
   */
  private static final int MINE_SUPPORT_PACK_TARGET = 32;
  ...
  public boolean turnWithTheDay() {
    ...
    Kithkyn.LOGGER.info("[road] '{}' wanders {} today", getFullName(), roamHeadingName());
    return true;
  }
   ...
   * whole person is remembered in the server's {@link com.quzzar.kithkyn.village.WandererPool}
  ...
  public void crossHorizon() {
    ...
    Kithkyn.LOGGER.info("'{}' passed beyond the horizon heading {}{}", getFullName(), roamHeadingName(),
  ...
  public void readAdditionalSaveData(CompoundTag compound) {
    ...
    this.wanderingStock = compound.contains("WanderingStock")
        ? com.quzzar.kithkyn.economy.EconomySnapshot.load(compound.getCompound("WanderingStock"))
        : null;
  ...
  public void aiStep() {
    ...
    if (!this.level().isClientSide && this.tickCount % 200 == 137
      ...
      if (village == null
        ...
        reloadState();
        Kithkyn.LOGGER.debug("[{}] was orphaned from their village and set out {}",
  ...
  public void tendSignatureGear() {
    ...
    for (SignatureGear.Piece piece : SignatureGear.of(getOccupation())) {
      ...
      logBlocker(blocker);
      Kithkyn.LOGGER.info("'{}' has no {} to keep in hand and the stores hold none",
          getFullName(), name);
    }
  }
  ...
  public com.quzzar.kithkyn.economy.EconomySnapshot getWanderingStock() {
    return this.wanderingStock;
  }

  public void setWanderingStock(com.quzzar.kithkyn.economy.EconomySnapshot stock) {
  ...
  private void stowPackAndRestock() {
    ...
    if (getOccupation() == Occupation.MINER) {
      ...
      // Dirt or stone for the day's flooring, lining, and vein plugs, topped up from the
  ...
  private ItemStack takeFromHomeChest(ItemStack want) {
    ...
    if (taken > 0) {
      ...
      Kithkyn.LOGGER.info("'{}' took {} {} from their chest at home for tomorrow's work",
  ...
  private void restockRations(BlockPos depositToLoc) {
    ...
    if (this.mealsCarried() == 0) {
      ...
      this.getVillage().logEvent(
          new com.quzzar.kithkyn.village.bookkeeping.NoResourceBookkeepingEvent(Items.BREAD, wanted));
    }
  }

  /**
   * A guard takes up a shield when one can be had, so {@link
   * com.quzzar.kithkyn.entities.ai.goals.RaiseShieldGoal} finally has something
   * to raise. The shield rides in the off hand and the guard swaps a meal in to eat
   * and the shield back after ({@link
   * com.quzzar.kithkyn.entities.ai.goals.PersonEatFoodGoal}); because this runs
  ...
  private void equipShieldToOffHand(ItemStack shield) {
    ...
    Kithkyn.LOGGER.info("[guard] {} takes up a shield", this.getFullName());
  }
  ...
  /**
   * Tops the miner's pack up from their home chest first and village storage
   * second, accepting the same dirt and stone family MineStep can actually place.
   */
  private void restockMineSupports(BlockPos depositToLoc) {
    int wanted = MINE_SUPPORT_PACK_TARGET - MineSupportMaterials.held(this.personMainInv);
    if (wanted <= 0) {
      return;
    }

    BlockPos chest = PersonalChest.of(this);
    Container home = chest == null ? null : PersonalChest.container(this, chest);
    if (home != null) {
      List<ItemStack> fromHome = MineSupportMaterials.take(home, wanted);
      for (ItemStack stack : fromHome) {
        wanted -= stack.getCount();
    ...
    for (Holder<Item> holder : BuiltInRegistries.ITEM.getTagOrEmpty(MineSupportMaterials.TAG)) {
      ...
      ItemStack got = this.getVillage().gatherItemStackFromVillage(
          new ItemStack(holder.value(), wanted), depositToLoc);
      if (!got.isEmpty()) {
        wanted -= got.getCount();
  ...
  private void maybeCraftBonemealFromBones(BlockPos depositToLoc) {
    ...
    int bonesHeld = com.quzzar.kithkyn.economy.VillagePricing.countHeld(this.getVillage(), Items.BONE);
    if (bonesHeld <= 0) {
      return;
    }
  ...
  public void tpToRest(String why) {
    ...
    Kithkyn.LOGGER.info("[unstuck] {} {}; brought to {} at {}",
  ...
  /** The exact defensive wall station this guard holds, when assigned one. */
  @Nullable
  ...
  protected void setVirtue(Virtue virtue, float value) {
    setData(KithkynAttachments.SOCIAL, getData(KithkynAttachments.SOCIAL).withVirtue(virtue, value));
  }

  public float getVirtue(Virtue virtue) {
    return getData(KithkynAttachments.SOCIAL).getVirtue(virtue);
  }
  ...
  protected void setFavoriteItems(Map<String, Double> favoriteItems) {
    setData(KithkynAttachments.SOCIAL, getData(KithkynAttachments.SOCIAL).withFavoriteItems(favoriteItems));
  }

  public Map<String, Double> getFavoriteItems() {
    return getData(KithkynAttachments.SOCIAL).favoriteItems();
  }

  protected void setRelationships(Map<UUID, Integer> relationships) {
    setData(KithkynAttachments.SOCIAL, getData(KithkynAttachments.SOCIAL).withRelationships(relationships));
  }

  public Map<UUID, Integer> getRelationships() {
    return getData(KithkynAttachments.SOCIAL).relationships();
  }
  ...
  public void openChat(ServerPlayer player) {
    ...
    if (getVillage() != null && level() instanceof net.minecraft.server.level.ServerLevel serverLevel
        && com.quzzar.kithkyn.wrongdoing.Standing.tierOf(getVillage(), serverLevel, player.getUUID())
            .atLeastAsBadAs(com.quzzar.kithkyn.wrongdoing.Standing.Tier.SHUNNED)) {
    ...
    com.quzzar.kithkyn.chat.PersonChatDispatcher.markOpen(this, player);
    ...
    List<com.quzzar.kithkyn.chat.ChatHistoryData.Exchange> priorExchanges = getData(
        KithkynAttachments.CHAT_HISTORY.get()).with(player.getUUID());
    boolean freshSession = getData(KithkynAttachments.CHAT_HISTORY.get())
        .staleFor(player.getUUID(), level().getDayTime());
    if (freshSession) {
      setData(KithkynAttachments.CHAT_HISTORY.get(),
          getData(KithkynAttachments.CHAT_HISTORY.get()).clearedFor(player.getUUID()));
    }
    ...
    boolean canTrade = isWanderingMerchant()
        ...
        : getOccupation() == Occupation.MERCHANT && getVillage() != null
            && com.quzzar.kithkyn.economy.Treasury.tradeBlocker(getVillage(),
    ...
    player.openMenu(new net.minecraft.world.SimpleMenuProvider(
        (containerId, inventory, opener) -> new com.quzzar.kithkyn.menu.MarketMenu(
    ...
    if (scrollback.isEmpty()) {
      ...
      com.quzzar.kithkyn.chat.PersonChatDispatcher.greet(this, player);
    }
  }
  ...
  public void pickFightWith(LivingEntity other) {
    ...
    setTarget(other);
    Kithkyn.LOGGER.info("'{}' picks a fight with {}", getFullName(), other.getName().getString());
  }
  ...
  public boolean holdsGrudgeAgainst(Player player) {
    return com.quzzar.kithkyn.relationships.OpinionService.opinionOf(this, player.getUUID())
        <= com.quzzar.kithkyn.configuration.KithkynConfig.GrudgeAttackBelow;
  }
  ...
  public boolean villageConsidersThreat(Player player) {
    ...
    threatVerdict = com.quzzar.kithkyn.wrongdoing.Standing
        .tierOf(getVillage(), serverLevel, player.getUUID())
        == com.quzzar.kithkyn.wrongdoing.Standing.Tier.HOSTILE;
    return threatVerdict;
  }
  ...
  protected void registerGoals() {
    ...
    Kithkyn.LOGGER.debug("REGISTERING GOALS FOR " + getUUID());
    ...
    this.goalSelector.addGoal(1, new com.quzzar.kithkyn.entities.ai.goals.VillageTravelGoal(this));
    this.goalSelector.addGoal(3, new com.quzzar.kithkyn.entities.ai.goals.PauseForConversationGoal(this));
    ...
    this.goalSelector.addGoal(3, new com.quzzar.kithkyn.entities.ai.goals.OpenFenceGateGoal(this));
    ...
    this.goalSelector.addGoal(4, new WorkLoopGoal<>(this,
        new com.quzzar.kithkyn.entities.ai.goals.work.ForageHuntStep()));
    this.goalSelector.addGoal(4, new WorkLoopGoal<>(this,
        new com.quzzar.kithkyn.entities.ai.goals.work.ForageChopStep()));
    this.goalSelector.addGoal(4, new WorkLoopGoal<>(this,
        new com.quzzar.kithkyn.entities.ai.goals.work.CampStep()));
    ...
    this.goalSelector.addGoal(5, new com.quzzar.kithkyn.entities.ai.goals.SeekConversationGoal(this, 0.5D));
