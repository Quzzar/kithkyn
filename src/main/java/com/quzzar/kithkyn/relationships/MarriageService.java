...
public final class MarriageService {
  ...
  private static void fileEmergentProposals(Village village, ServerLevel level) {
    ...
    for (UUID id : village.getPopulation()) {
      ...
      if (chosen != null && !MarriageProposals.hasProposed(village, id, chosen)) {
        ...
        Kithkyn.LOGGER.info("[marriage] '{}' wishes to marry {} in '{}'",
        ...
        situation, List.of(APPROVE, DECLINE)).whenComplete((result, error) -> {
          ...
          level.getServer().execute(() -> {
            if (error != null) {
              ...
              Kithkyn.LOGGER.error("[marriage] '{}' could not decide a marriage", village.getName(), error);
              return;
            }
            if (result != null && result.isPresent() && result.get().choiceIndex() == 0) {
              ...
              if (result != null && result.isPresent()) {
                Kithkyn.LOGGER.info("[marriage] '{}' set aside a marriage for now: {}",
  ...
  private static void wed(Village village, ServerLevel level, UUID idA, UUID idB, String chosenSurname, String reason) {
    ...
    Kithkyn.LOGGER.info("[marriage] '{}' and '{}' are wed in '{}' as the {} household: {}",
