...
public final class VillageGoal {
  ...
  private static final String KEY_MODE = "goal_construction_mode";
  private static final String KEY_REDEVELOPMENT = "goal_redevelopment";
  ...
  public static void set(Village village, String buildingName, String reason, String shortfall,
      int villageTime) {
    set(village, buildingName, ConstructionMode.FRESH, reason, shortfall, villageTime);
  }

  /** Names a goal and preserves whether it is saving for a new site or an upgrade. */
  public static void set(Village village, String buildingName, ConstructionMode mode, String reason,
      String shortfall, int villageTime) {
    CompoundTag strategy = village.getBrain().getStrategy();
    strategy.putString(KEY_BUILDING, buildingName);
    strategy.putString(KEY_MODE, mode.name());
    ...
    strategy.remove(KEY_REDEVELOPMENT);
    Kithkyn.LOGGER.info("Village '{}' is saving for {}: {}", village.getName(), buildingName, reason);
  }

  /** Saving retains the same reviewed site and removal list, rather than silently choosing new victims. */
  public static void set(Village village, ConstructionChoice choice, String reason, String shortfall,
      int villageTime) {
    set(village, choice.info().getName(), choice.mode(), reason, shortfall, villageTime);
    if (choice.redevelopment() != null) {
      village.getBrain().getStrategy().put(KEY_REDEVELOPMENT, RedevelopmentPlan.CODEC
          .encodeStart(net.minecraft.nbt.NbtOps.INSTANCE, choice.redevelopment()).getOrThrow());
    }
  }

  @Nullable
  public static RedevelopmentPlan redevelopment(Village village) {
    var saved = village.getBrain().getStrategy().get(KEY_REDEVELOPMENT);
    return saved == null ? null : RedevelopmentPlan.CODEC.parse(net.minecraft.nbt.NbtOps.INSTANCE, saved)
        .resultOrPartial(error -> Kithkyn.LOGGER.error("Cannot read redevelopment goal for '{}': {}",
            village.getName(), error)).orElse(null);
  }
  ...
  /** The saved construction mode, or null for a goal written by an older version. */
  @Nullable
  public static ConstructionMode mode(Village village) {
    String name = village.getBrain().getStrategy().getString(KEY_MODE);
    if (name.isEmpty()) {
      return null;
    }
    try {
      return ConstructionMode.valueOf(name);
    } catch (IllegalArgumentException ignored) {
      return null;
    }
  }

  public static void setMode(Village village, ConstructionMode mode) {
  ...
  public static void clear(Village village, String why) {
    CompoundTag strategy = village.getBrain().getStrategy();
    if (!strategy.getString(KEY_BUILDING).isEmpty()) {
      Kithkyn.LOGGER.info("Village '{}' is no longer saving for {} ({})",
  ...
  public static void markStalled(Village village, String buildingName, int villageTime) {
    ...
    Kithkyn.LOGGER.info("Village '{}' gave up on {} for now: nothing came in for it the whole time",
