...
public final class VillageTrading {
  ...
  public static boolean consider(Village village, ServerLevel level) {
    ...
    Kithkyn.LOGGER.debug("Village '{}' is weighing {} trades with the bank",
  ...
  private static void settle(Village village, ServerLevel level, List<Deal> deals,
    ...
    if (index == deals.size()) {
      Kithkyn.LOGGER.info("Village '{}' traded nothing with the bank: {}",
  ...
  private static ItemStack shortfall(Village village, StructureInProgress project) {
    ConstructionQuote quote = ConstructionQuote.captureProject(village, project, village.stockTally());
    return quote.missing().isEmpty() ? null : quote.missing().get(0);
  }

  /** The first cost item this saved goal needs more of than the village holds. */
  private static ItemStack shortfall(Village village, BuildingInfo info) {
    if (info == null) {
      return null;
    }
    ConstructionQuote quote = ConstructionQuote.captureGoal(village, info, village.stockTally());
    return quote.missing().isEmpty() ? null : quote.missing().get(0);
  }
  ...
  private static void execute(Village village, ServerLevel level, Deal deal, String reason) {
    if (deal.selling()) {
      ...
      Kithkyn.LOGGER.info("Village '{}' sold {} {} to the bank for {} emeralds: {}",
    ...
    Kithkyn.LOGGER.info("Village '{}' bought {} {} from the bank for {} emeralds: {}",
  ...
  private static int costOf(Village village, BuildingInfo info, Item item) {
    ...
    List<ItemStack> required = mode == null
        ? ConstructionQuote.requiredFor(village, info)
        : ConstructionQuote.requiredFor(info, mode);
    return new ConstructionQuote(required, List.of()).requiredFrom(item);
  }
