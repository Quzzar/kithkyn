package com.quzzar.kithkyn.client.appearance;
...
public final class PersonAppearanceTextures implements ResourceManagerReloadListener {
  ...
  public ResourceLocation textureFor(Person person) {
    try {
      ...
      if (!loggedFailure) {
        Kithkyn.LOGGER.error("Could not compose person appearance; using the default player texture", exception);
        loggedFailure = true;
      }
  ...
  private NativeImage loadLayer(ResourceManager resources, AppearanceAsset asset, AppearancePart part)
      throws IOException {
    ResourceLocation location = ResourceLocation.fromNamespaceAndPath(Kithkyn.MODID, asset.texturePath(part));
