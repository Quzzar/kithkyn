...
public class KithkynConfig {
    ...
    public static class CommonConfig {
        ...
        public CommonConfig(ModConfigSpec.Builder builder) {
            ...
            LlmEnabled = builder.comment("Enable the LLM behind villager conversation, village decisions, personas, and settlement names. The offline model (about 2 GB) downloads from HuggingFace on first server start and is cached in <game dir>/kithkyn/models. Budget roughly 3 GB of RAM beyond the game's needs for the offline model, or point LLM provider at a cloud service for no local cost. Check /kithkyn status in-game.").translation(Kithkyn.MODID + ".config.LlmEnabled").define("Enable LLM?", true);
            LlmProviderName = builder.comment("Which LLM answers for the villagers: 'local' (default) downloads llama.cpp and an offline model and runs it for you, with no account or installation; 'claude', 'openai', or 'deepseek' use that cloud service with your API key. An invalid name leaves the LLM unavailable and never starts the local model. Changing this requires a game restart.").translation(Kithkyn.MODID + ".config.LlmProvider").define("LLM provider", "local");
            LlmApiKey = builder.comment("Optional; only needed when 'LLM provider' is a cloud service (claude/openai/deepseek). The offline 'local' provider ignores it. Paste it here in plain text and TREAT THIS FILE LIKE A PASSWORD: anyone with this file can spend your account's money.").translation(Kithkyn.MODID + ".config.LlmApiKey").define("LLM API key", "");
            LlmCloudModel = builder.comment("Model id for the cloud provider (unused by the 'local' provider, which uses LLM local model). Defaults to OpenAI's gpt-5.6-luna. Set the corresponding model id when using Claude or DeepSeek; an empty value uses that provider's built-in default.").translation(Kithkyn.MODID + ".config.LlmCloudModel").define("LLM cloud model", "gpt-5.6-luna");
    ...
    public static class AdvancedConfig {
        ...
        public AdvancedConfig(ModConfigSpec.Builder builder) {
            ...
            builder.comment("Advanced LLM sampling. The everyday LLM settings (provider, key, models, whether villagers chat) live in kithkyn-common.toml.").push("llm");

            LlmChatMaxNewTokens = builder.comment("Maximum tokens a villager may generate per line of conversation. Kept modest because generation time scales with it: on the smallest model 47 tokens took about 13 seconds in-game, so a low cap bounds the worst-case pause. Raise it to let villagers say more.").translation(Kithkyn.MODID + ".config.LlmChatMaxNewTokens").defineInRange("LLM chat max new tokens", 64, 16, 1024);
            LlmChatTemperature = builder.comment("Sampling temperature for conversation. Higher is livelier and more varied; too high on a small model makes it ramble or break the JSON its reply is wrapped in. 0.4 is the tuned default.").translation(Kithkyn.MODID + ".config.LlmChatTemperature").defineInRange("LLM chat temperature", 0.4D, 0.0D, 2.0D);
            LlmDecisionMaxNewTokens = builder.comment("Maximum tokens the LLM may generate per village decision (which building to start, which idle camper to hire). A decision is a short JSON answer, so this stays small.").translation(Kithkyn.MODID + ".config.LlmDecisionMaxNewTokens").defineInRange("LLM decision max new tokens", 128, 16, 1024);
            LlmDecisionTemperature = builder.comment("Sampling temperature for village decisions. Low keeps them focused; the rules already narrow every choice to legal options, so a little variety here just adds character. 0.0 is fully deterministic.").translation(Kithkyn.MODID + ".config.LlmDecisionTemperature").defineInRange("LLM decision temperature", 0.1D, 0.0D, 2.0D);
            ...
            AttractivenessBase = builder.comment("Attractiveness starting value before any inputs apply.").translation(Kithkyn.MODID + ".config.AttractivenessBase").defineInRange("Attractiveness base", 50.0D, 0.0D, 100.0D);
            AttractivenessFoodMax = builder.comment("Maximum attractiveness bonus from stocked food.").translation(Kithkyn.MODID + ".config.AttractivenessFoodMax").defineInRange("Attractiveness food max", 25.0D, 0.0D, 100.0D);
            AttractivenessFoodTargetPerCapita = builder.comment("Edible items per villager at which the food bonus reaches its maximum.").translation(Kithkyn.MODID + ".config.AttractivenessFoodTargetPerCapita").defineInRange("Attractiveness food target per capita", 8.0D, 0.1D, 1000.0D);
            AttractivenessFreeBedsMax = builder.comment("Maximum attractiveness bonus from free-bed headroom.").translation(Kithkyn.MODID + ".config.AttractivenessFreeBedsMax").defineInRange("Attractiveness free beds max", 10.0D, 0.0D, 100.0D);
            AttractivenessFreeBedsTarget = builder.comment("Number of free beds at which the headroom bonus reaches its maximum.").translation(Kithkyn.MODID + ".config.AttractivenessFreeBedsTarget").defineInRange("Attractiveness free beds target", 2.0D, 1.0D, 100.0D);
            AttractivenessHomelessMax = builder.comment("Maximum attractiveness penalty when every villager is homeless.").translation(Kithkyn.MODID + ".config.AttractivenessHomelessMax").defineInRange("Attractiveness homeless max", 20.0D, 0.0D, 100.0D);
            AttractivenessDeathWeight = builder.comment("Attractiveness penalty per unit of decaying death-event impact.").translation(Kithkyn.MODID + ".config.AttractivenessDeathWeight").defineInRange("Attractiveness death weight", 8.0D, 0.0D, 100.0D);
            AttractivenessHurtWeight = builder.comment("Attractiveness penalty per unit of decaying hurt-by-player impact.").translation(Kithkyn.MODID + ".config.AttractivenessHurtWeight").defineInRange("Attractiveness hurt weight", 3.0D, 0.0D, 100.0D);
            AttractivenessShortageWeight = builder.comment("Attractiveness penalty per unit of decaying resource-shortage impact.").translation(Kithkyn.MODID + ".config.AttractivenessShortageWeight").defineInRange("Attractiveness shortage weight", 2.0D, 0.0D, 100.0D);
            AttractivenessTheftWeight = builder.comment("Attractiveness penalty per unit of decaying theft impact. Being robbed makes a village less appealing to move to, on the same scale as its other griefs.").translation(Kithkyn.MODID + ".config.AttractivenessTheftWeight").defineInRange("Attractiveness theft weight", 1.0D, 0.0D, 100.0D);
            AttractivenessArriveThreshold = builder.comment("Score above which new villagers arrive (consumed by the arrival system).").translation(Kithkyn.MODID + ".config.AttractivenessArriveThreshold").defineInRange("Attractiveness grow threshold", 50.0D, 0.0D, 100.0D);
            AttractivenessEmigrateThreshold = builder.comment("Score below which villagers leave (consumed by the emigration system).").translation(Kithkyn.MODID + ".config.AttractivenessEmigrateThreshold").defineInRange("Attractiveness decline threshold", 25.0D, 0.0D, 100.0D);
            ...
            StandingHostileBelow = builder.comment("Standing at or below which a village's guards attack you on sight. Standing is the average of its residents' opinions, from -100 to 100.").translation(Kithkyn.MODID + ".config.StandingHostileBelow").defineInRange("Standing hostile below", -70, -100, 100);
            StandingShunnedBelow = builder.comment("Standing at or below which nobody in the village will approach or talk to you.").translation(Kithkyn.MODID + ".config.StandingShunnedBelow").defineInRange("Standing shunned below", -50, -100, 100);
            ...
            StandingDislikedBelow = builder.comment("Standing at or below which the village charges you over the odds, rising the further you fall.").translation(Kithkyn.MODID + ".config.StandingDislikedBelow").defineInRange("Standing disliked below", -10, -100, 100);
            ...
            PopulationCheckIntervalSeconds = builder.comment("Seconds between a village's arrival/emigration checks (phase-staggered per village).").translation(Kithkyn.MODID + ".config.PopulationCheckIntervalSeconds").defineInRange("Population check interval seconds", 100, 5, 86400);
            MinimumVillagePopulation = builder.comment("People a village keeps whatever its mood, both ways: emigration never takes the roster below this, and a village pushed below it (by deaths) grows back up to it regardless of attractiveness or resources.").translation(Kithkyn.MODID + ".config.MinimumVillagePopulation").defineInRange("Minimum village population", 4, 0, 64);
            ShortageEventCooldownSeconds = builder.comment("Minimum seconds between resource-shortage events logged by a village's planner.").translation(Kithkyn.MODID + ".config.ShortageEventCooldownSeconds").defineInRange("Shortage event cooldown seconds", 600, 10, 86400);
            ArrivalEdgeMinDistance = builder.comment("Minimum distance from the village center at which newcomers appear (grows with building spread).").translation(Kithkyn.MODID + ".config.ArrivalEdgeMinDistance").defineInRange("Arrival edge min distance", 32, 8, 256);
            TravelTimeoutSeconds = builder.comment("Seconds a newcomer or emigrant may spend walking before the village snaps them to their destination.").translation(Kithkyn.MODID + ".config.TravelTimeoutSeconds").defineInRange("Travel timeout seconds", 90, 10, 3600);
            ...
            FamilyFirstTalkDelayDays = builder.comment("Minecraft days a newly housed married couple lives together before their first family-planning conversation.").translation(Kithkyn.MODID + ".config.FamilyFirstTalkDelayDays").defineInRange("First family talk delay days", 1, 0, 365);
            FamilyTalkRetryDays = builder.comment("Minecraft days before a couple who said not now, or whose conversation could not finish, discusses children again.").translation(Kithkyn.MODID + ".config.FamilyTalkRetryDays").defineInRange("Family talk retry days", 4, 1, 365);
            FamilyBirthCooldownDays = builder.comment("Minecraft days after a birth before the parents can discuss having another child.").translation(Kithkyn.MODID + ".config.FamilyBirthCooldownDays").defineInRange("Family birth cooldown days", 8, 1, 365);
            TwinBirthChance = builder.comment("Chance that an agreed birth produces identical twins. Triplets are rolled separately first.").translation(Kithkyn.MODID + ".config.TwinBirthChance").defineInRange("Twin birth chance", 0.03D, 0.0D, 0.5D);
            TripletBirthChance = builder.comment("Chance that an agreed birth produces identical triplets.").translation(Kithkyn.MODID + ".config.TripletBirthChance").defineInRange("Triplet birth chance", 0.0025D, 0.0D, 0.1D);

            builder.pop();

            builder.comment("Labor: how jobs are filled from the idle pool and reshuffled.").push("labor");

            JobSwapThreshold = builder.comment("Minimum aptitude improvement (on the 3-18 stat scale) before the village reassigns a job to someone better suited. Higher = less churn.").translation(Kithkyn.MODID + ".config.JobSwapThreshold").defineInRange("Job swap threshold", 3.0, 0.5, 15.0);
            JobSwapIntervalSeconds = builder.comment("Seconds between job-swap evaluation passes (phase-staggered per village).").translation(Kithkyn.MODID + ".config.JobSwapIntervalSeconds").defineInRange("Job swap interval seconds", 60, 10, 3600);
            JobSwapCooldownDays = builder.comment("Game days a person is protected from further job swaps after being placed, swapped, or displaced (also gates the hungry-village move onto an open field).").translation(Kithkyn.MODID + ".config.JobSwapCooldownDays").defineInRange("Job swap cooldown days", 3.0, 0.0, 30.0);
            BuildCooldownDays = builder.comment("Game days a village waits after finishing a building before starting the next; it spends the lull grading terrain and laying paths instead of overbuilding.").translation(Kithkyn.MODID + ".config.BuildCooldownDays").defineInRange("Build cooldown days", 2.0, 0.0, 30.0);
            RedevelopmentEnabled = builder.comment("Allow complete construction proposals that dismantle specific blocking village buildings. Actual paid materials return at 50%; free and legacy buildings have no refund. The planner checks surviving builders, housing and staffed food production before asking the model.").define("Allow village redevelopment", true);

            builder.pop();

            builder.comment("Economy: the always-available exchange that floors and caps every price.").push("economy");

            BankSpread = builder.comment("How punishing the always-available exchange is: it pays value/spread for goods and charges value*spread. Higher means trading with players and other villages matters more.").translation(Kithkyn.MODID + ".config.BankSpread").defineInRange("Bank spread", 4.0, 1.0, 32.0);
            ...
            DeveloperCommands = builder.comment("Registers the /kkdev command tree: scaffolding, diagnostics, and stand-ins for interfaces that do not exist yet. Off for players; on for anyone developing the mod.").translation(Kithkyn.MODID + ".config.DeveloperCommands").define("Developer commands", false);
